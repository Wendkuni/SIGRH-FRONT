...
export class ConvenancePersonnelleNiveauNationaleComponent implements OnInit{
  ...
  save(){
    if(this.listPieceJustificatif.length > 0){
      ...
      this.messageService.add({severity:'error', summary: 'Erreur', detail: 'Aucune pièce justificative n\'a été ajoutée'});
    }
  }
  ...
  createAffectation(){
      ...
      this.mobiliteService.createAffectationByConvenance(data,this.listPieceJustificatif).subscribe( () =>{
        this.messageService.add({severity:'success', summary: 'Succès', detail: 'Affectation enregistrée avec succès'});
        this.cancelMobilite();
      },
        error => {
          this.messageService.add({severity: 'error', summary: 'Error', detail: 'Affectation non enregistrée', life: 3000});
        });
  }
